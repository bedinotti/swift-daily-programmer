...
func maximizeProfit(input: String) -> String {
    ...
    for (index, buyPrice) in prices[prices.startIndex..<prices.endIndex.advancedBy(-2)].enumerate() {
        if let sellPrice = prices[index.advancedBy(2)..<prices.endIndex].maxElement() {
...
testMethod(maximizeProfit, withInput: simpleInput, expectingOutput: simpleOutput)
testMethod(maximizeProfit, withInput: longInput, expectingOutput: longOutput)
